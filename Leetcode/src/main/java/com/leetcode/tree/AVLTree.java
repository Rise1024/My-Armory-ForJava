 ...
 *
 *
 * 二叉树本身是无序的，只规定的节点的定义，二叉查找树才规定了有序，
 * 对于二叉查找树的每个节点，左子树中的元素必须小于或等于其父节点中的键(L <P)，右子树中的元素必须大于或等于其父节点中的键( R> P)。
 * 因为有序，查询时就可以使用二分法查找数据，在理想的左右完全平衡的情况下，查找时间复杂度为O(log N)，插入删除的时间复杂度也为O(log N)。
 * 但很多时候插入的顺序并不是理想状态，可能会导致树偏斜，最差情况树变成了一个链表，查找复杂度就为O(N)了，
 * 故需要通过平衡提升二叉树的查找性能，就有了AVL树（平衡二叉树）以及红黑树
 *
 * AVL树的特性：一棵AVL树是其每个结点的左子树和右子树的高度最多相差1的二叉查找树(空树的高度为-1)，这个差值也称为平衡因子（
 *
 * 构造平衡二叉搜索树(AVL树)
 * ① 在结点X的左孩子结点的左子树中插入元素
 * ② 在结点X的左孩子结点的右子树中插入元素
