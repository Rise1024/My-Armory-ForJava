package com.example.log.desensitization.utils;
...
public class DesensitizationUtil {
     ...
     */
    public static final Pattern REGEX_PATTERN = Pattern.compile("\\s*([\"]?[\\w]+[\"]?)(\\s*[:=]+[^\\u4e00-\\u9fa5_\\-@,.*/{\\[\\w]*\\s*)([\\u4e00-\\u9fa5_\\-@.*/:\\[\\w]+)[\\W&&[^\\-@.]]?\\s*");
    ...
    public static final String SYMBOLEQUAL="=\"";
    ...
    public String customChange(String eventFormattedMessage) {
        try {
            //去除转译引号的影响
            String originalMessage = eventFormattedMessage.replaceAll("\\\\\"", "").trim();;
            boolean flag = false;
            // 获取Yml配置文件内容 - Map格式
            ...
            if (!CollectionUtils.isEmpty(patternMap)) {
                ...
                // 获取一个原始Message的正则匹配器
                Matcher regexMatcher = REGEX_PATTERN.matcher(originalMessage);
                        ...
                        if (null != keyPatternValue && null != originalValue && !"null".equals(originalValue)) {
                            ...
                            if (!"null".equals(value) || value.equalsIgnoreCase(key)) {
                                ...
                                if(PASSWORD.equalsIgnoreCase(patternVales)){
                                    ...
                                    String noNullRegex = regexMatcher.group(2).replaceAll("\\s", "");
                                    originalMessage = originalMessage.replace(origin, regexMatcher.group(1) + noNullRegex + "******");
                                    /** 针对全脱敏情况
                                          处理特殊字符串
                                     */
                                    if (noNullRegex.startsWith(SYMBOLCOLON)||noNullRegex.startsWith(SYMBOLEQUAL)){
                                        int indexBegin = originalMessage.indexOf(regexMatcher.group(1)) + regexMatcher.group(1).length() + 2;
                                        int indexEnd = originalMessage.indexOf("\"", indexBegin);
                                        originalMessage = originalMessage.replace(originalMessage.substring(indexBegin, indexEnd), "******");
                                    }
                                    flag = true;
