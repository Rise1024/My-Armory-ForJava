package com.base.datastructure.collection.array;
...
public class ArrayList<E> extends AbstractList<E>
    ...
    /**
     * 最小容量minCapacity的计算：数组内的元素数量加1
     *
     * 判断是否需要扩容：
     * 扩容的条件 需要的最小空间大于数组的内存空间
     * 1:对于初始化空数组的list minCapacity最小容量默认为10 首次添加元素
     * 数组容量为0肯定<所需最小容量minCapacity10,满足扩容条件，需要扩容
     * 2:对于初始化不为空数组的list minCapacity最小容量默认为1 首次添加元素
     * 如果数组容量<所需最小容量minCapacity1,满足扩容条件，需要扩容
     * 如果数组容量>所需最小容量minCapacity1,不满足扩容条件，不需要扩容
     * 也就是说只要初始化的数组容量大于1,首次添加不需要扩容，直到数组内元素数量大于初始化的数组容量
     *
     * 扩容的机制：
     * 将数组的容量增加到原来的1.5倍
     * 然后判断所需最小容量minCapacity是否大于第一次扩容后数组容量
     * 如果所需最小容量minCapacity>第一次扩容后数组容量 则设置数组容量为最小容量minCapacity
     *
     *
     *
     *
     ...
     * 查找元素的索引，根据索引copy出新的数组
     * 因为有查找索引的的原因，时间复杂度为O（n）
     *
    ...
    private void fastRemove(int index) {
        ...
        int numMoved = size - index - 1;
        if (numMoved > 0)
            //将elementData源数组中 从位 index 到 numMoved之间的数值 copy 到 elementData目标数组中,在目标数组的第index+1位开始放置.
            System.arraycopy(elementData, index + 1, elementData, index,
                    numMoved);
        // 使size-1 ，设置elementData的size位置为空，让GC来清理内存空间
        elementData[--size] = null; //便于垃圾回收器回收
